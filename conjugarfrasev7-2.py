...
def cambiageneroynumero(adjective, gender=MALE):
    ...
    # normal => normales
    if PLURAL in gender and not is_vowel(w[-1:]) and not w.endswith(("es", "as", "os","or")):
        return w + "es"
    ...
    # profesor -> profesora
    if w.endswith("or"):
        if FEMALE in gender and SINGULAR in gender:
            return w+"a"
        if FEMALE in gender and PLURAL in gender:
            return w+"as"
        if PLURAL in gender:
...
def cambiaconcordanciasustantivo(data,pi,pf,gender):
    for pos,item in enumerate(data):
        if pos >= pi and pos <= pf:
            if ('VMP' in item[2]):
                agn=cambiageneroynumero(item[0],gender=gender)
...
def cambiaconcordanciasadjetivo(data,pi,pf,gender):
    for pos,item in enumerate(data):
        if pos >= pi and pos <= pf:
            if ('VMP' in item[2]):
                agn=cambiageneroynumero(item[0],gender=gender)
...
def cambiaconcordanciadeterminante(data,pi,pf,gender):
    for pos,item in enumerate(data):
        if pos>=pi and pos <=pf:
...
def concordanciacopulativa(data,pi,pf,posps,posv):
...
# ahora miramos la concordancia del adjetivo o atributo y su determinante
    ...
    data=cambiaconcordanciadeterminante(data,posv,pf[0],gender)
    data=cambiaconcordanciasadjetivo(data,posv,pf[0],gender)
#    st.write(data)    

    return data

def concordanciasustantivodeterminante(data,pi,pf):
    ...
    #                        gender=sgn[0].lower()
                ...
                if gender[0:2]!=dgn.lower():
                    data=cambiaconcordanciadeterminante(data,pi,pf,gender)
    #            data=cambiaconcordanciasustantivo(data,pi,pf,gender,debug)
                if agn!='XX':
                    data=cambiaconcordanciasadjetivo(data,pi,pf,gender)
        else:
    ...
    #                        gender=sgn[0].lower()
        ...
        if num!='X':
            ...
            if num!='1':
                    gender=gender+PLURAL
            data=cambiaconcordanciasustantivo(data,pi,pf,gender)
            data=cambiaconcordanciasadjetivo(data,pi,pf,gender)
        else:
...
def concordanciadeterminantesustantivo(data,pi,pf):
...
def concordanciaexcepcionfemenino(data,pi,pf):
...
def concordanciadeterminantenombre(data,pi,pf):
    pos=pi
    while pos<(pf-1):
        item=data[pos]
        if item[2][0]=='D':
            if data[pos+1][2][0]=='N' or data[pos+1][2][0]=='V':
                if data[pos+1][0] in atonicafemeninos:
                    data=concordanciaexcepcionfemenino(data,pos,pos+1)
                else:
                    data=concordanciasustantivodeterminante(data,pos,pos+1)
#                    data=concordanciadeterminantesustantivo(data,pos,pos+1,debug)
        pos=pos+1   
    return data

def concordancianombreadjetivo(data,pi,pf):
    pos=pi
    while pos<(pf-1):
        item=data[pos]
        if item[2][0]=='N':
...
def buscanombrespropios(data,nombres_masculinos,nombres_femeninos,pi,pf):
    ...
    for pos,item in enumerate(data):
        if pos >= pi and pos <= pf:
            if lema!='D' and (item[2][0]!="V" or pos==pi or lema=='C' or lema=='F' or lema=='A'):
                if pos<=posv:
#            if pos==pi:
...
def es_oracionfinal(data,pi,posv):
...
#   take the first and last position of the segment
            ...
            if es_interrogativa(data,pi,pf):
                tipofrase='INT'
# al iniciar una nueva frase ponemos conjugar a True ya que puede que en la anterior se hubiera puesto a False
            conjugar=True
...
#   Check the concordance of the determinant and sustantivs
            data=concordanciadeterminantenombre(data,pi,pf)
 
#   Check the concordance of the adjective and sustantivs
            data=concordancianombreadjetivo(data,pi,pf)
            pp=posv+1

#   Concordance in segments with preposition after the verb 
            for pos in pospc:
                if pos>=pp:
                    data=concordanciasustantivodeterminante(data,pp,pos)
                    pp=pos+1
...
#   We have to check if the verb is a sustantive and it is preceded by a determinante. If so, we change the tag to verb
            ...
            if es_frasecopulativa(data,pi,pf):
                cgs=concordanciacopulativa(data,pi,pospc,posps,posv)
...
#   if the main sentence is in past perfect, the second sentence is in past imperfect
                    ...
                    if verbo in verbos3p:
                        penum=[3,SG]
                if es_oracionfinal(data,pi-2,posv):
                    mood=SUBJUNCTIVE
                    nextverbsubj=True
...
#   Test if the previous segment ends in "para" and the next segment starts with "que". In this case the verb of the next segment is in subjunctive mood
            if es_oracionfinal(data,pf,posv):
                nextverbsubj=True
...
fout=open('logflexionarV7.txt','a')
